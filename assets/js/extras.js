...
<div id="dwclock">
  ...
  <div id="hour"><div class="hand"></div></div>
    ...
    .grain{position:absolute;mix-blend-mode:difference;background-image:url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' version='1.1' width='96' height='96'><defs><filter id='grain' x='0' y='0' width='1' height='1'><feTurbulence type='turbulence' baseFrequency='.9' numOctaves='9' seed='98' stitchTiles='stitch' x='-1%' y='-1%' width='102%' height='102%' result='turbulence'/><feSpecularLighting surfaceScale='9' specularConstant='9' specularExponent='9' lighting-color='%23ffffff' x='-1%' y='-1%' width='102%' height='102%' in='turbulence' result='specularLighting'><feDistantLight azimuth='9' elevation='98'/></feSpecularLighting></filter></defs><rect width='98' height='98' fill='%23000000' id='blackbody' x='-1' y='-1' opacity='.03' /><rect width='98' height='98' fill='%23ffffff' filter='url(%23grain)' x='-1' y='-1' opacity='.03' /></svg>");width:100%;height:100%}#dwclock{opacity:.33;margin:auto;width:100vmin;height:100vmin;filter:blur(2vmin) saturate(2)}#hour,#min{position:absolute;width:100vmin;height:100vmin}.hand{--min:40vmin;--hour:28vmin;--tsf:translateY(calc(50vmin - var(--min)));margin:0 auto auto;border-right:2vmin solid transparent;border-bottom:var(--min) solid #60f;border-left:2vmin solid transparent;border-radius:2vmin;background-image:linear-gradient(0deg,var(--bg) 0%,#60f 100%);width:3vmin;height:var(--min)}#hour .hand{--tsf:translateY(calc(50vmin - var(--hour)));border-bottom:var(--hour) solid #20f;background-image:linear-gradient(0deg,var(--bg) 0%,#20f 100%);height:var(--hour)}
    `
...
if (content && toc && (content.getBoundingClientRect().x > halfmarginpar) ) {
    ...
    const observer = new IntersectionObserver(entries => {
        ...
        if (entry.intersectionRatio > 0) {
            document.querySelector(`#has-TableOfContents a[href="#${id}"]`).parentNode.className = 'act';
        } else {
            document.querySelector(`#has-TableOfContents a[href="#${id}"]`).parentNode.className = '';
        }
        });
    });
